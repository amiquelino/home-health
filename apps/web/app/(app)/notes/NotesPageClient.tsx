...
function PatientNotes({ patient }: { patient: HHPatient }) {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto p-6">
      ...
      {showForm && (
        ...
        >
          <h3 className="text-sm font-semibold text-slate-700">Nova evolução</h3>
          {error && (
    ...
    <div className="flex h-full gap-0 -m-6">
      ...
      <div className="w-64 shrink-0 border-r border-slate-200 flex flex-col bg-white">
        <div className="p-4 border-b border-slate-100">
          <h1 className="text-base font-bold text-slate-900 mb-3">Evoluções</h1>
          <input
            type="search"
            value={q}
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {selected ? (
          <PatientNotes key={selected.id} patient={selected} />
        ) : (
          <div className="flex-1 flex items-center justify-center">
            <p className="text-slate-400 text-sm">Selecione um paciente para ver as evoluções.</p>
          </div>
        )}
      </div>
    </div>
