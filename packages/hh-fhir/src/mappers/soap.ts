...
export function toFHIRClinicalImpression(note: SOAPNote): ClinicalImpression {
  return {
    ...
    extension: [
      { url: HH_EXT.NOTE_TYPE, valueString: 'evolucao' },
...
export function toFHIRAnamnesis(note: AnamnesisNote): ClinicalImpression {
  return {
    resourceType: 'ClinicalImpression',
    ...(note.id && { id: note.id }),
    status: 'completed',
    ...
    ...(note.appointmentId && { encounter: { reference: `Encounter/${note.appointmentId}` } }),
    date: note.date,
    note: [{ text: `[QP] ${note.chiefComplaint}\n[HDA] ${note.presentIllness}\n[HPP] ${note.pastHistory}\n[OBJ] ${note.objective}` }],
    extension: [
      { url: HH_EXT.NOTE_TYPE,     valueString: 'avaliacao' },
      { url: HH_EXT.ANAMNESIS_CC,  valueString: note.chiefComplaint },
      { url: HH_EXT.ANAMNESIS_HDA, valueString: note.presentIllness },
      { url: HH_EXT.ANAMNESIS_HPP, valueString: note.pastHistory },
      { url: HH_EXT.ANAMNESIS_OBJ, valueString: note.objective },
    ],
  };
}

export function fromFHIRAnamnesis(fhir: ClinicalImpression): AnamnesisNote {
  return {
    id: fhir.id,
    patientId: fhir.subject?.reference?.split('/')[1] ?? '',
    practitionerId: fhir.assessor?.reference?.split('/')[1] ?? '',
    date: fhir.date ?? new Date().toISOString(),
    chiefComplaint: getExtension(fhir as any, 'ANAMNESIS_CC')  ?? '',
    presentIllness: getExtension(fhir as any, 'ANAMNESIS_HDA') ?? '',
    pastHistory:    getExtension(fhir as any, 'ANAMNESIS_HPP') ?? '',
    objective:      getExtension(fhir as any, 'ANAMNESIS_OBJ') ?? '',
  };
}
